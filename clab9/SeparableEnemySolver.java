    ...
    public boolean isSeparable() {
        ...
        for (String label: g.labels())
            if (!groups.containsKey(label)) {
                if (!isSeparableHelper(label, null, groups, 1))
                    return false;
            }
        return true;
    }

    private boolean isSeparableHelper(String label, String edgeTo, HashMap<String, Integer> groups, int group) {
        if (groups.containsKey(label))
            return groups.get(label) == group;

        groups.put(label, group);
        for (String neighbor: g.neighbors(label)) {
            if (!neighbor.equals(edgeTo)) {
                if (!isSeparableHelper(neighbor, label, groups, -1*group))
                    return false;
            }
        }
        return true;
